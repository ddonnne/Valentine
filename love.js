...
function explodeBody() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      piece.style.background = `url('yh.png') -${x*pieceWidth}px -${y*pieceHeight}px`;
      ...
      piece.style.transform = `translate(${Math.cos(angle)*dist}px, ${Math.sin(angle)*dist}px) rotate(${Math.random()*720}deg)`;
      piece.style.opacity = 0;
      });
    }
  }

  // go to next page after explosion
  setTimeout(() => {
...
window.onload = () => {
  ...
  cards.forEach((card, i) => {
    cardData.push({
      element: card,
      position: 0,                       // starting left
      top: 10 + i * 25,                  // vertical spacing (vh)
      speed: 1 + Math.random() * 2       // random speed
    });

    card.style.top = cardData[i].top + "vh";
    card.style.left = cardData[i].position + "px";
  });
  ...
  function animate() {
    cardData.forEach(c => {
      ...
      // bounce left/right within container
      if (c.position + c.element.offsetWidth >= containerWidth || c.position <= 0) {
        c.speed = -c.speed;
      }

      c.element.style.left = c.position + "px";
    });
